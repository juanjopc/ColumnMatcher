...
model = genai.GenerativeModel(
    model_name="gemini-exp-1206",  # O el modelo que estés utilizando
...
def process_block(source_json, batch, source_col_name, target_col_name, batch_idx, total_batches):
    ...
    user_message = (
        f"Each row in the target list must receive its own match in the source list if an adequate match is found.\n\n"
        f"If you do not find a close enough match (60% similarity or more), return an empty value (“”) for 'source'.\n\n"
        f"Preserve the original spelling and formatting.\n\n"
        f"Return only a valid JSON array of objects, where each object has the structure:\n\n"
        f"{{\n"
        f"  \"target\": \"original_value_from_target\",\n"
        f"  \"source\": \"matched_value_from_source_or_blank\"\n"
        f"}}\n\n"
        f"If there is no suitable match for an entry, return an empty string (\"\") for 'source'. "
        ...
        f"Here is the source list: {source_json}\n\n"
        f"Here are the entries to match: {batch_json}."
    )
